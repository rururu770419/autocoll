...
def get_all_categories(store_id):
    """全カテゴリを取得（store_idでフィルタ）"""
...
def update_category_label(store_id, field_key, new_label):
...
def get_field_options(store_id, field_key):
    """指定カテゴリの選択肢を取得（store_idでフィルタ）"""
    conn = get_db_connection()
    try:
        with conn.cursor(row_factory=dict_row) as cursor:
            cursor.execute("""
...
def get_all_field_options(store_id):
    """全カテゴリの選択肢を取得（顧客編集画面用、store_idでフィルタ）"""
    conn = get_db_connection()
    try:
        with conn.cursor(row_factory=dict_row) as cursor:
            cursor.execute("""
...
def add_field_option(store_id, field_key, option_value, display_color='#f0f0f0', store_code=None):
    ...
    try:
        with conn.cursor() as cursor:
            # 重複チェック
            cursor.execute("""
                SELECT id FROM customer_field_options
                WHERE store_id = %s AND field_key = %s AND option_value = %s
            """, (store_id, field_key, option_value))

            if cursor.fetchone():
                return {'success': False, 'error': 'duplicate', 'message': 'この選択肢は既に存在します'}
            ...
            cursor.execute("""
                ...
                (store_id, store_code, field_key, option_value, display_color, display_order)
                VALUES (%s, %s, %s, %s, %s, %s)
                RETURNING id
            """, (store_id, store_code, field_key, option_value, display_color, next_order))
...
def update_field_option(option_id, store_id, option_value=None, display_color=None):
...
def toggle_field_option_visibility(option_id, is_hidden, store_id):
...
def delete_field_option(store_id, option_id):
    """選択肢を削除（使用中チェック付き、store_idでフィルタ）"""
    conn = get_db_connection()
    try:
        with conn.cursor() as cursor:
            # 使用中かチェック
            ...
            cursor.execute(f"""
                ...
                WHERE store_id = %s AND {column_name} = %s
            """, (store_id, option_value))
...
def move_field_option(store_id, option_id, direction):
    """
    選択肢の表示順序を変更（store_idでフィルタ）
        ...
        with conn.cursor() as cursor:
            ...
            if direction == 'up':
                ...
                cursor.execute("""
                    ...
                    LIMIT 1
                """, (store_id, field_key, current_order))
            else:  # down
